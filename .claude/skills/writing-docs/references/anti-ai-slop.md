...
- **"This" + abstract noun:** "This approach enables...", "This methodology provides...", "This framework ensures...". Name the actual thing. Instead of "This approach enables," write "The recipe table enables" or merge with the previous sentence.
...
> The compactor validates input before processing. This prevents invalid sources from being attached. The validation uses the same rules as the public API.

Better (varied):

> Validation runs before any source is attached, using the same rules as the public API. Invalid sources never see a target file.

## Concreteness over abstraction

Never write "improved performance" when you can write "skipped HNSW rebuild on `--skip-hnsw` cuts peak RAM from several GiB to flat." Never write "better error handling" when you can name the exception class and the condition that triggers it. Developers want specifics: class names, config keys, method signatures, version numbers. Abstraction is filler.

Bad: "This enhancement significantly improves the developer experience."

Better: "`replace_with_compacted()` now raises `FileExistsError` when `<source>.bak` already exists instead of overwriting silently."
...
Bad: "This was an oversight. `compact_database` just didn't propagate the metric."

Better: "`compact_database` discarded the HNSW metric on rebuild; the new path recovers it from `pragma_hnsw_index_info()`."
